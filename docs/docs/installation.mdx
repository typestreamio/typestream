...
Load sample data (books, authors, ratings, users) into your Kafka cluster:

```bash
typestream local seed
```

Verify the topics are available:

```bash
echo 'ls /dev/kafka/local/topics' | typestream
```

You should see topics like `books`, `authors`, `ratings`, `users`, and `_schema`.
