...
#      -> the server provisions the Qdrant sink connector automatically
#   5. wait for all docs to land in the collection
# ---------------------------------------------------------------------------
set -euo pipefail
...
export KAFKA_CONNECT SERVER_ADDR QDRANT_REST SCHEMA_REGISTRY COLLECTION
...
qdrant_count() {
  curl -s "${QDRANT_REST}/collections/${COLLECTION}/points/count" \
    -H 'Content-Type: application/json' -d '{"exact":true}' \
    | jq -r '.result.count // 0'
}
docs_ready() { [ "$(qdrant_count)" -ge "${EXPECTED_DOCS}" ]; }
...
echo "[5/6] applying TypeStream pipeline (server provisions the Qdrant sink connector)..."
...
register_sink_connector   # no-op for the native qdrantSink (see target.sh)

echo "[6/6] waiting for all ${EXPECTED_DOCS} docs to land in ${COLLECTION}..."
retry 180 "${EXPECTED_DOCS} docs in ${COLLECTION}" docs_ready
echo "  ${COLLECTION} now has $(qdrant_count) points"
